package com.kota.akshay.accplotwithfilt;
...
public class AccelerationSensorActivity extends Activity implements SensorEventListener
{
    ...
    private SimpleXYSeries filtHistorySeries = null;

    ArrayList<Double> xFiltData = new ArrayList<>();
    ArrayList<Double> yFiltData = new ArrayList<>();
    ArrayList<Double> zFiltData = new ArrayList<>();
    ArrayList<Double> accFiltData = new ArrayList<>();
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        filtHistorySeries = new SimpleXYSeries("FiltData");
        filtHistorySeries.useImplicitXVals();
        ...
        accDataPlot.addSeries(filtHistorySeries,
                new LineAndPointFormatter(this, R.xml.linestyle));
    ...
    public synchronized void onSensorChanged(SensorEvent sensorEvent) {
        ...
        /* Using ArrayList to store the previous values
           to be used for SG Filtering
         */
        if (xFiltData.size() == sgWindow)
        {
            xFiltData.remove(0);
            xFiltData.add(x);
            yFiltData.remove(0);
            ...
            zFiltData.remove(0);
            zFiltData.add(z);
            accFiltData.remove(0);
            accFiltData.add(magnitude);
        }
        else
        {
            xFiltData.add(x);
        ...
        /* Calculating local mean of the data */
        double sum = 0;
        for (int i = 0; i < accFiltData.size(); i++)
        {
            sum += accFiltData.get(i);
        }

        double mean = sum / sgWindow;
        double[] filtInput = new double[sgWindow];

        /* Subtracting mean from the magnitude of the data */
        for (int i = 0; i < accFiltData.size(); i++)
        {
            filtInput[i] = accFiltData.get(i) - mean;
        }

        /* Using CurveSmooth class of Flanagan's Library
           for SG Filtering
         */
        CurveSmooth sgolay = new CurveSmooth(filtInput);
        double[] filtOutput = sgolay.savitzkyGolay(sgWindow);
        ...
        filtHistorySeries.addLast(null, filtOutput[sgWindow / 2]);
    }
